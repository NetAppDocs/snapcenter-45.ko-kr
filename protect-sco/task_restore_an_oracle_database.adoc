...
복구는 구성된 아카이브 로그 위치에서 사용할 수 있는 아카이브 로그를 사용하여 수행됩니다. 복구에 필요한 아카이브 로그를 구성된 위치에서 사용할 수 없는 경우 로그를 포함하는 스냅샷 복사본을 마운트하고 경로를 외부 아카이브 로그로 지정해야 합니다.
...
외부 아카이브 로그 위치를 지정하려는 경우
 a| 
외부 아카이브 로그 위치 지정 * 을 선택한 다음 외부 아카이브 로그 파일의 위치를 지정합니다.
